...
(function ($) {
  $.fn.simpleLoadMore = function (options) {
    ...
    var settings = $.extend({
      ...
      btnHTML: '',
      item: '',
      onAllLoaded: function () {} // 添加一个回调函数，用于在所有内容加载完后执行自定义操作
    }, options);
    ...
    $loadMore.each(function (i, el) {
      ...
      // var hiddenBtn = settings.btnHTML ? settings.btnHTML : '<a href="#" class="close_btn">Hide<i class="fas fa-angle-down"></i></a>';
      var $btnHTML = $(btnHTML);
      ...
      // $btn2 = $thisLoadMore.find('.close_btn');
      ...
      $btn.on('click', function (e) {
        ...
        if ($updatedItems.length < settings.count) {
          $this.remove();
          settings.onAllLoaded($thisLoadMore); // 当所有内容加载完后调用回调函数
          // var $hiddenBtn = $(hiddenBtn);
          // $thisLoadMore.append($hiddenBtn);
        }
      });
      if ($items.length <= settings.count) {
        settings.onAllLoaded($thisLoadMore);
    }
    });
  }
}(jQuery));
